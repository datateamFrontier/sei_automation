...
"""
Program ini berisi fungsi-fungsi untuk membuat data indeks SEI
"""
...
def drop_null_row(data_mentah):
    ...
    data_mentah.dropna(subset=data_mentah.iloc[:,3:].columns, axis=0, how='all',inplace=True)
    data_mentah.iloc[:,3:] = data_mentah.iloc[:,3:].fillna(0)
    return data_mentah



def get_ivr_nonivr(list_brand, kriteria_call_center):
    """
    ...
    """
    if kriteria_call_center=='car insurance':
        brand_nonivr = [list_brand[1]]
    elif kriteria_call_center=='courier':
        brand_nonivr = list_brand[4:]
    elif kriteria_call_center=='regular banking':
...
def crosstab_rata_transpose(data_mentah, list_brand,
                    nomor_bulan=nomor_bulan,nama_bulan=nama_bulan):
    """
    ...
    data_rata_industri.iloc[:,2:] = round(data_rata_industri.iloc[:,2:]*100,2)
    ...
    crosstab_rata_transpose = crosstab_rata.transpose()
    crosstab_rata_transpose.index = crosstab_rata_transpose.index.rename('Aspek')
    crosstab_rata_transpose = crosstab_rata_transpose.reset_index()
    return crosstab_rata_transpose
...
def nilai_sub_aspek(kriteria, daftar_bobot, list_bulan, list_brand, crosstab_rata_transpose,
                    kriteria_call_center=None, nomor_bulan=nomor_bulan, nama_bulan=nama_bulan):
    """
    Melakukan penghitungan nilai rincian untuk mendapatkan nilai subaspek dari data berkriteria 'call center' atau 'twitter'.
    Data 'email' tidak memiliki level nilai sampai rincian.
    
    Parameter
    ---------
    kriteria : kriteria SEI, diisi 'call center' atau 'twitter' sebagai penentu perhitungan bobot
    ...
    crosstab_rata_transpose : dataframe yang sudah jadi berisi rata-rata penilaian KPI per brand tiap bulan
    kriteria_call_center : hanya berpengatuh pada kriteria 'call center' untuk menentukan sub-kriterianya
    nomor_bulan : daftar nomor bulan
    nama_bulan : daftar nama-nama bulan
    ...
    for bln in list_bulan:
        for nm_file in list_brand:
            for i in range(len(key_group)):
                ...
                if kriteria == 'call center':
                    if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                        nilai = tb_bobot.loc[min(id_key):max(id_key),
                                             'BOBOT RINCIAN NON-IVR'].dot(crosstab_rata_transpose.loc[min(id_key):max(id_key),
                                                                                                                  (bln,nm_file)])
                        nilai = [tb_bobot['NOMOR LEVEL 1'].values[0], key_group[i], bln, nm_file, round(nilai,2)]
                    else:
                        nilai = tb_bobot.loc[min(id_key):max(id_key),
                                             'BOBOT RINCIAN IVR'].dot(crosstab_rata_transpose.loc[min(id_key):max(id_key),
                                                                                                                  (bln,nm_file)])
                        nilai = [tb_bobot['NOMOR LEVEL 1'].values[0], key_group[i], bln, nm_file, round(nilai,2)]
                    ...
                    nilai = tb_bobot.loc[min(id_key):max(id_key),
                                             'BOBOT RINCIAN'].dot(crosstab_rata_transpose.loc[min(id_key):max(id_key),
                                                                                                                  (bln,nm_file)])
                    nilai = [tb_bobot['NOMOR LEVEL 1'].values[0], key_group[i], bln, nm_file, round(nilai,2)]
                    nilai_subaspek.append(nilai)
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        nilai_subaspek[(bln,'~Industri')] = round(nilai_subaspek[(bln)].mean(axis=1),2)
    nilai_subaspek = nilai_subaspek.stack().unstack()
...
def nilai_aspek_with_sub(kriteria, daftar_bobot, list_bulan, list_brand, crosstab_rata_transpose, nilai_subaspek,
    ...
    Parameter
    ---------
    kriteria : kriteria SEI, diisi 'call center','email', atau 'twitter' sebagai penentu perhitungan bobot
    daftar_bobot : dataframe bobot
    list_bulan : list array bulan yang ada di dalam dataframe
    list_brand : list brand yang ada di dalam dataframe
            ...
            for i in range(len(key_group)):
                ...
                if kriteria == 'call center':
                    if key_group[i]==37:
                        ...
                        if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),'BOBOT SUB ASPEK NON-IVR'].reset_index(
                                drop=True).dot(nilai_subaspek.loc[:, (bln,nm_file)])
                        else:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),'BOBOT SUB ASPEK IVR'].reset_index(
                                drop=True).dot(nilai_subaspek.loc[:, (bln,nm_file)])
                    elif key_group[i]==62:
                        id_key = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin([key_group[i]])].index.values
                        ...
                        if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),
                                                 'BOBOT SUB ASPEK NON-IVR'].reset_index(drop=True).dot(i_62)
                        else:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),
                                                 'BOBOT SUB ASPEK IVR'].reset_index(drop=True).dot(i_62)
                    else:
                        id_key = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin([key_group[i]])].index.values
                        if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),'BOBOT SUB ASPEK NON-IVR'].dot(
                                crosstab_rata_transpose.loc[min(id_key):max(id_key), (bln,nm_file)])
                        else:
                            nilai = tb_bobot.loc[min(id_key):max(id_key),'BOBOT SUB ASPEK IVR'].dot(
                                crosstab_rata_transpose.loc[min(id_key):max(id_key), (bln,nm_file)])
                    nilai = round(nilai,2)
                ...
                # TWITTER ---------------------
                    ...
                    if key_group[i]==37:
                        ...
                        nilai = tb_bobot.loc[min(id_key):max(id_key),
                                                 'BOBOT SUB ASPEK'].reset_index(drop=True).dot(ab)
                    else:
                        id_key = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin([key_group[i]])].index.values
                        nilai = tb_bobot.loc[min(id_key):max(id_key),'BOBOT SUB ASPEK'].dot(
                            crosstab_rata_transpose.loc[min(id_key):max(id_key), (bln,nm_file)])
                    nilai = round(nilai,2)
            ...
            for i in range(len(key_group)):
                if key_group[i] in nilai_aspek['Nomor'].unique():
                    ...
                    if len(df_temp)==1:
                        ...
                        elif kriteria == 'twitter' or kriteria == 'email':
                            nao = round(nao, 2)
                            nilai = [tb_bobot['DIMENSI'][id_key].values[0],tb_bobot['KPI'][id_key].values[0],
...
def df_aspek_pivot(nilai_aspek, nilai_aspek_only, list_bulan):
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        df_aspek_pivot[(bln,'~Industri')] = round(df_aspek_pivot[(bln)].mean(axis=1),2)
    df_aspek_pivot = df_aspek_pivot.stack().unstack()
...
def tabel_summary(df_pivot, list_bulan):
    ...
    df_pivot_avg = df_pivot_avg.stack()
    df_pivot_avg.columns = nama_bulan[:len(list_bulan)]
    df_pivot_avg['~Average'] = round(df_pivot_avg.mean(axis=1),2)
...
def df_subkpi_pivot(kriteria, daftar_bobot, list_bulan, list_brand, df_aspek_pivot,
    ...
    for bln in list_bulan:
        for nm_file in list_brand:
            for i in key_group:
                ...
                if kriteria == 'call center':
                    if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                        nilai_subkpi_ = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin(
                            id_key)]['BOBOT ASPEK NON-IVR'].reset_index(drop=True).dot(aspek_)
                    else:
                        nilai_subkpi_ = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin(
                            id_key)]['BOBOT ASPEK IVR'].reset_index(drop=True).dot(aspek_)

                elif kriteria == 'twitter' or kriteria == 'email':
                    nilai_subkpi_ = tb_bobot[tb_bobot['NOMOR LEVEL 1'].isin(
                        id_key)]['BOBOT ASPEK'].reset_index(drop=True).dot(aspek_)

                nilai_subkpi_ = round(nilai_subkpi_,2)
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        df_subkpi_pivot[(bln,'~Industri')] = round(df_subkpi_pivot[(bln)].mean(axis=1),2)
    df_subkpi_pivot = df_subkpi_pivot.stack().unstack()
...
def df_kpi_pivot(kriteria, daftar_bobot, list_bulan, list_brand, df_subkpi_pivot,
                ...
                if kriteria == 'call center':
                    if i=='Accessibility' or i=='Availability' or i=='Connection Speed':
                        ...
                        if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                            ...
                            nilai_kpi_ = temp_bobot['BOBOT SUB KPI NON-IVR'].reset_index(drop=True).dot(
                                temp_data[(bln,nm_file)].reset_index(drop=True))
                        else:
                            temp_bobot = temp_bobot[temp_bobot['BOBOT SUB KPI IVR'].notnull()].sort_values(by='SUB KPI')
                            temp_data = i_kpi[i_kpi['SUB KPI'].isin(daftar_sub)].sort_values(by='SUB KPI')
                            nilai_kpi_ = temp_bobot['BOBOT SUB KPI IVR'].reset_index(drop=True).dot(
                                temp_data[(bln,nm_file)].reset_index(drop=True))

                else:
                    if ((kriteria == 'twitter') and (i=='Probing' or i=='Providing Solution' or i=='Closing')) or ((kriteria=='email') and (i=='Feasibility' or i=='Accessibility' or i=='Availability')):
                        nilai_kpi_ = i_kpi[i_kpi['SUB KPI']==i][(bln,nm_file)].values[0]
                    else:
                        temp_bobot = tb_bobot[tb_bobot['KPI'].isin([i])]
                        daftar_sub = list(temp_bobot['SUB KPI'].unique())
                        ...
                        temp_data = i_kpi[i_kpi['SUB KPI'].isin(daftar_sub)].sort_values(by='SUB KPI')
                        nilai_kpi_ = temp_bobot['BOBOT SUB KPI'].reset_index(drop=True).dot(
                            temp_data[(bln,nm_file)].reset_index(drop=True))

                nilai_kpi_ = round(nilai_kpi_,2)
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        df_kpi_pivot[(bln,'~Industri')] = round(df_kpi_pivot[(bln)].mean(axis=1),2)
    df_kpi_pivot = df_kpi_pivot.stack().unstack()
...
def df_dimensi_pivot(kriteria, daftar_bobot, list_bulan, list_brand, df_kpi_pivot,
            ...
            for i in key_group:
                ...
                if kriteria == 'call center':
                    if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[1]:
                        ...
                        nilai_dimensi_ = temp_bobot['BOBOT KPI NON-IVR'].reset_index(drop=True).dot(
                            temp_data[(bln,nm_file)].reset_index(drop=True))
                    else:
                        temp_bobot = temp_bobot[temp_bobot['BOBOT KPI IVR'].notnull()].sort_values(by='KPI')
                        temp_data = i_dimensi[i_dimensi['KPI'].isin(daftar_sub)].sort_values(by='KPI')
                        nilai_dimensi_ = temp_bobot['BOBOT KPI IVR'].reset_index(drop=True).dot(
                            temp_data[(bln,nm_file)].reset_index(drop=True))

                elif kriteria == 'twitter' or kriteria == 'email':
                    temp_bobot = temp_bobot[temp_bobot['BOBOT KPI'].notnull()].sort_values(by='KPI')
                    temp_data = i_dimensi[i_dimensi['KPI'].isin(daftar_sub)].sort_values(by='KPI')
                    nilai_dimensi_ = temp_bobot['BOBOT KPI'].reset_index(drop=True).dot(
                        temp_data[(bln,nm_file)].reset_index(drop=True))

                nilai_dimensi_ = round(nilai_dimensi_,2)
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        df_dimensi_pivot[(bln,'~Industri')] = round(df_dimensi_pivot[(bln)].mean(axis=1),2)
    df_dimensi_pivot = df_dimensi_pivot.stack().unstack()
...
def df_ccsei_pivot(kriteria, daftar_bobot, list_bulan, list_brand, df_dimensi_pivot,
    ...
    for bln in list_bulan:
        for nm_file in list_brand:
            ...
            if kriteria == 'call center':
                if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[0]:
                    indeks_final = tb_bobot['BOBOT DIMENSI IVR'].reset_index(drop=True).dot(
                    i_indeks.reset_index(drop=True))
                else:
                    indeks_final = tb_bobot['BOBOT DIMENSI NON-IVR'].reset_index(drop=True).dot(
                    i_indeks.reset_index(drop=True))

            elif kriteria == 'twitter' or kriteria == 'email':
                indeks_final = tb_bobot['BOBOT DIMENSI'].reset_index(drop=True).dot(
                i_indeks.reset_index(drop=True))

            indeks_final = round(indeks_final,2)
    ...
    df_ccsei_pivot = df_ccsei.pivot_table(index='Brand',columns=['Bulan'], values='Indeks')
    df_ccsei_pivot = df_ccsei_pivot.transpose()
    df_ccsei_pivot['~Industri'] = round(df_ccsei_pivot.mean(axis=1),2)
    df_ccsei_pivot = df_ccsei_pivot.transpose()
    df_ccsei_pivot.columns = nama_bulan[:len(list_bulan)]
    df_ccsei_pivot['~Average'] = round(df_ccsei_pivot.mean(axis=1),2)
...
def eng_calltwit(kriteria, bobot_engagement, df_aspek_pivot, list_bulan, list_brand,
                kriteria_call_center=None):
    """
    Melakukan penghitungan nilai engagement khusus untuk kriteria 'call center' dan 'twitter'
    ...
    df_eg_kpi_pivot = df_eg_kpi.pivot_table(index=['Dimensi','KPI'], columns=['Bulan','Brand'],values='Indeks')
    for bln in nomor_bulan[:len(list_bulan)]:
        df_eg_kpi_pivot[(bln,'~Industri')] = round(df_eg_kpi_pivot[(bln)].mean(axis=1),2)
    df_eg_kpi_pivot = df_eg_kpi_pivot.stack().unstack()
            ...
            for dimensi_ in list_dimensi:
                ...
                if kriteria == 'call center':
                    if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[0]:
                        ...
                        nilai_ = round(bbt_.dot(val_eg),2)
                    else:
    ...
    for bln in nomor_bulan[:len(list_bulan)]:
        df_eg_dimensi_pivot[(bln,'~Industri')] = round(df_eg_dimensi_pivot[(bln)].mean(axis=1),2)
    df_eg_dimensi_pivot = df_eg_dimensi_pivot.stack().unstack()
    ...
    for bln in list_bulan:
        for nm_file in list_brand:
            ...
            if kriteria == 'call center':
                if nm_file in get_ivr_nonivr(list_brand, kriteria_call_center)[0]:
                    i_final = bbt['BOBOT DIMENSI IVR'].reset_index(drop=True).dot(
                    i_eng.reset_index(drop=True))
                else:
                    indeks_final = bbt['BOBOT DIMENSI NON-IVR'].reset_index(drop=True).dot(
                    i_eng.reset_index(drop=True))

            elif kriteria == 'twitter':
                i_final = bbt['BOBOT DIMENSI'].reset_index(drop=True).dot(
                i_eng.reset_index(drop=True))
            i_final = round(i_final,2)
    ...
    df_eng_final_pivot = df_eng_final.pivot_table(index='Brand',columns=['Bulan'], values='Indeks')
    df_eng_final_pivot = df_eng_final_pivot.transpose()
    df_eng_final_pivot['~Industri'] = round(df_eng_final_pivot.mean(axis=1),2)
    df_eng_final_pivot = df_eng_final_pivot.transpose()
    df_eng_final_pivot.columns = nama_bulan[:len(list_bulan)]
    df_eng_final_pivot['~Average'] = round(df_eng_final_pivot.mean(axis=1),2)
...
def eng_email(kriteria, bobot_engagement, df_aspek_pivot, list_bulan, list_brand, crosstab_rata_transpose):
    ...
    for i in bobot_engagement['NOMOR LEVEL 1'].unique():
        ...
        if i==7 or i==12:
            ...
            temp_kode = tempe['KODE NOMOR'].values
            tempe2 = crosstab_rata_transpose[crosstab_rata_transpose['Aspek'].isin(temp_kode)].copy()
            tempe2 = tempe2.rename(columns = {'Aspek':'Kode'})
    ...
    df_eg_subkpi_pivot = df_eg_subkpi.pivot_table(index=['Dimensi','KPI','SUB KPI'], columns=['Bulan','Brand'],values='Indeks')

    for bln in nomor_bulan[:len(list_bulan)]:
        df_eg_subkpi_pivot[(bln,'~Industri')] = round(df_eg_subkpi_pivot[(bln)].mean(axis=1),2)
    df_eg_subkpi_pivot = df_eg_subkpi_pivot.stack().unstack()
    ...
    for i in df_eg_dimensi.Bulan.unique():
        ...
        df_eg_dimensi.Bulan = df_eg_dimensi.Bulan.replace(i,no_bln)
    df_eg_dimensi = df_eg_dimensi.reset_index(drop=True)
    df_eg_dimensi_pivot = df_eg_dimensi.pivot_table(index=['Dimensi'], columns=['Bulan','Brand'],values='Indeks')
    ...
    for bln in list_bulan:
        for nm_file in list_brand:
            i_eng = df_eg_dimensi_pivot.reset_index()[(bln,nm_file)]
            i_final = bbt['BOBOT DIMENSI'].reset_index(drop=True).dot(i_eng.reset_index(drop=True))
            i_final = round(i_final,2)
            engg_final.append([bln, nm_file, i_final])
    df_eng_final = pd.DataFrame(engg_final, columns=['Bulan','Brand','Indeks'])
...
def engagement(kriteria, bobot_engagement, df_aspek_pivot, list_bulan, list_brand, crosstab_rata_transpose=None,
    ...
    list_bulan : list array bulan yang ada di dalam dataframe
    list_brand : list brand yang ada di dalam dataframe
    crosstab_rata_transpose : data dari dataframe ini akan digunakan sebagai nilai subaspek jika kategori SEI tidak punya nilai subaspek hasil perhitungan dari rincian, jika kriteria 'call center' atau 'twitter' maka tidak perlu (None)
    kriteria_call_center : hanya berpengatuh pada kriteria 'call center' untuk menentukan sub-kriterianya
...
def area_perbaikan(kriteria, data_mentah, bobot_perbaikan, list_bulan, list_brand,
    ...
    """            
    kolom_perbaikan = list(bobot_perbaikan['KODE NOMOR'])
    ...
    tb_perbaikan.index = tb_perbaikan.index.rename('KODE NOMOR')
    tb_perbaikan = tb_perbaikan.reset_index()

    aspek_perbaikan = pd.concat([bobot_perbaikan.iloc[:,0:4], tb_perbaikan.iloc[:,1:]], axis=1)
    aspek_perbaikan = aspek_perbaikan.set_index(['NOMOR','KODE NOMOR','SUB KPI','ASPEK'])
    ...
    df_aspek_perbaikan = aspek_perbaikan.stack().unstack()
...
def tabel_indeks(tabel_summary_kpi, tabel_summary_dimensi, list_bulan):
    """
    Membuat tabel indeks berdasarkan KPI dan DIMENSI
    
    Parameter
    ---------
    tabel_summary_kpi : dataframe indeks kpi dengan kolom satu level (bulan saja)
    tabel_summary_dimensi : dataframe indeks kpi dengan kolom satu level (bulan saja)
    
    Return
    ------
    index_by_kpi : dataframe indeks kpi dengan kolom nama kpi
    index_by_dimensi : dataframe indeks dimensi dengan kolom nama dimensi
    """

    temp_kpi = tabel_summary_kpi.copy()
    temp_dimensi = tabel_summary_dimensi.copy()

    temp_kpi = temp_kpi.reset_index()
    temp_kpi = temp_kpi.drop(['Dimensi']+list_bulan, axis=1)
    index_by_kpi = temp_kpi.pivot(index='Brand',columns='KPI')

    temp_dimensi = temp_dimensi.reset_index()
    temp_dimensi = temp_dimensi.drop(list_bulan, axis=1)
    index_by_dimensi = temp_dimensi.pivot(index='Brand',columns='Dimensi')

    return  index_by_kpi, index_by_dimensi